...
static std::optional<APInt> evaluateIntegerLikeConstant(Value value) {
  ...
  std::optional<unsigned> resultWidth = getIntegerLikeBitWidth(value.getType());
...
static bool addUnsignedOverflow(uint64_t first, uint64_t second,
                                uint64_t &result) {
  const bool overflows = first > std::numeric_limits<uint64_t>::max() - second;
  if (overflows) {
    return true;
  }
  result = first + second;
...
LogicalResult PTOIRTranslator::RecursionIR(Region *region) {
  ...
  auto result = region->walk<WalkOrder::PreOrder>([&](Operation *op) {
    ...
    // --- Case C: 控制流 (SCF) ---
      ...
      // the same GM payload.
      if (failed(UpdatePTOOpInfoWithPipeline(op, pto::PipelineType::PIPE_S,
      ...
      if (auto tpush = dyn_cast<pto::TPushOp>(op);
        ...
        if (failed(UpdatePTOOpInfoWithPipeline(talloc,
                                               pto::PipelineType::PIPE_S))) {
          return WalkResult::interrupt();
        }
    ...
    const bool hasUnmodeledEffect = options_.failOnUnmodeledEffects &&
                                    !modeledOperation &&
...
LogicalResult PTOIRTranslator::UpdateHelperCallInfo(func::CallOp callOp) {
  const bool hasMemoryOperand =
      llvm::any_of(callOp.getOperands(), [](Value operand) {
  ...
  if (options_.failOnUnmodeledEffects && hasMemoryOperand &&
      !hasPreciseEffects) {
    callOp.emitError(
        "helper memory-effect annotations are missing or malformed");
    return failure();
  }
  ...
  for (const auto &parentInfo : buffer2MemInfoMap_[source]) {
    ...
    for (auto [offset, stride] : llvm::zip(*offsets, parentInfo->byteStrides)) {
      ...
      const bool overflows =
          ...
          addUnsignedOverflow(partitionOrigin, dimensionOffset, updatedOrigin);
      if (overflows) {
        overflow = true;
        break;
      }

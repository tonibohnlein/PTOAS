...
static std::optional<uint64_t> addByteOffset(uint64_t base, int64_t offset) {
  if (offset >= 0)
    return addByteOffset(base, static_cast<uint64_t>(offset));
...
getContiguousMemRefSizeInBytes(MemRefType type) {
  if (!type.hasStaticShape() || type.getRank() != kTileRank2D)
    return std::nullopt;

  SmallVector<int64_t> strides;
  int64_t offset = ShapedType::kDynamic;
  if (failed(mlir::pto::getPTOMemRefStridesAndOffset(type, strides, offset)) ||
  ...
  if (shape[0] <= 0 || shape[1] <= 0 || strides[0] < 0 || strides[1] < 0)
    return std::nullopt;
  bool rowMajorContiguous =
      strides[1] == 1 && (shape[0] == 1 || strides[0] == shape[1]);
  bool colMajorContiguous =
      strides[0] == 1 && (shape[1] == 1 || strides[1] == shape[0]);
  if (!rowMajorContiguous && !colMajorContiguous)
    return std::nullopt;
  ...
  if (cols > std::numeric_limits<uint64_t>::max() / rows)
    return std::nullopt;
  uint64_t elements = rows * cols;
  if (elemBytes > std::numeric_limits<uint64_t>::max() / elements)
    return std::nullopt;
...
static bool canRetainSubviewParentRange(Value source,
  ...
  if (!sourceType)
    return false;
  auto contiguousSize = getContiguousMemRefSizeInBytes(sourceType);
  return contiguousSize && parentInfo.allocateSize >= *contiguousSize;
}
...
LogicalResult PTOIRTranslator::UpdatePointerCastOpMemInfo(pto::PointerCastOp op) {
  ...
  if (op.getAddrs().size() == 1) {
    ...
    // identity. Local pointer_cast operands are absolute physical addresses:
    // keep a constant byte address for cross-root range analysis, and mark a
    // dynamic address as unknown-absolute so alias analysis stays conservative.
    // Downstream view ops accumulate their delta into baseAddresses[0].
    Value rootSrc = op.getAddrs().front();
    SmallVector<uint64_t> baseAddresses{0};
    AddressProvenance addressProvenance = AddressProvenance::RootRelative;
    if (isLocalAddressSpace(space)) {
      addressProvenance = AddressProvenance::UnknownAbsolute;
      if (std::optional<uint64_t> address = getKnownPhysicalAddress(rootSrc)) {
        baseAddresses[0] = *address;
        addressProvenance = AddressProvenance::KnownAbsolute;
      }
    }
    auto newMemInfo = std::make_unique<BaseMemInfo>(
  ...
  for (Value a : op.getAddrs()) {
    ...
    if (!address) {
      ...
      auto newMemInfo = std::make_unique<BaseMemInfo>(
          ...
          isLocalAddressSpace(space) ? AddressProvenance::UnknownAbsolute
...
void PTOIRTranslator::UpdateAliasBufferInfo(Value result, Value source) {
  ...
  int64_t newSize = -1;
  bool isDynamicReinterpretOffset = false;
  ...
  for (auto &parentInfo : buffer2MemInfoMap_[source]) {
    ...
    // A first static reinterpret_cast offset applies independently to every
    // underlying alternative multi-buffer slot. It cannot be accumulated from
    // addresses already adjusted by a subview, because reinterpret_cast
    // replaces descriptor metadata relative to the underlying allocation.
    // Dynamic offsets, segmented views, and chained reinterpret_casts likewise
    // cannot be represented exactly by the current address list.
    bool isReinterpret =
        isa_and_nonnull<memref::ReinterpretCastOp>(result.getDefiningOp());
    bool cannotModelReinterpretExactly =
        isReinterpret &&
        (isDynamicReinterpretOffset || newInfo->hasAppliedSubviewOffset ||
    ...
    if (cannotModelReinterpretExactly) {
      ...
      if (std::optional<uint64_t> address = addByteOffset(0, deltaOffset))
        newInfo->baseAddresses.push_back(*address);
      else {
        markAddressRangeUnknown(*newInfo);
        addressRangeUnknown = true;
      }
...
void PTOIRTranslator::UpdateConservativeSubviewAliasBufferInfo(Value result,
                                                               Value source) {
  if (!result || !source) return;
  if (!buffer2MemInfoMap_.contains(source)) return;
...
void PTOIRTranslator::UpdateSlotMarkerAliasBufferInfo(pto::SlotMarkerOp op) {
  ...
  for (auto &parentInfo : buffer2MemInfoMap_[source]) {
    ...
    if (newInfo->addressListKind == AddressListKind::AlternativeSlots &&
        isConstSlot && constSlotIdx >= 0 &&

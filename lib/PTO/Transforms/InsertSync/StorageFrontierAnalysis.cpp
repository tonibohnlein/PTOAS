...
namespace {
// Test adapter for the same native engine used by InsertSync. The test input
// explicitly provides ownership; production still bypasses authored events.
struct PTOExperimentHandoffPlanningPass :
    ...
    StringRef getDescription() const final { return "Test native guarded handoff construction and realization"; }
    Option<bool> useMmad{*this, "mmad-chains", llvm::cl::init(false), llvm::cl::desc("Qualified MMAD ordering")};
    PTOExperimentHandoffPlanningPass() = default;
    PTOExperimentHandoffPlanningPass(const PTOExperimentHandoffPlanningPass& other) : PassWrapper(other) { useMmad = other.useMmad; }
    void runOnOperation() override {
        auto function = getOperation();
        if (function.isDeclaration()) return;
        SmallVector<Operation*> events, barriers;
        function.walk([&](Operation* op) {
            if (isa<SetFlagOp, WaitFlagOp>(op)) events.push_back(op);
            if (isa<BarrierOp>(op) && op->hasAttr("pto.handoff_test_owned")) barriers.push_back(op);
        });
        Budget budget;
        auto result = planInsertSyncHandoffs(function, events, barriers, useMmad, budget);
        function.emitRemark("InsertSync handoff experiment: ") << result.advanced << " advanced, " << result.delayed
            << " delayed, " << result.split << " split; " << result.setsRemoved << " sets / " << result.waitsRemoved
            << " waits removed; " << result.attempts << " trials; " << result.reason;
        if (result.internalError) { function.emitError(result.reason); signalPassFailure(); }
    }
};

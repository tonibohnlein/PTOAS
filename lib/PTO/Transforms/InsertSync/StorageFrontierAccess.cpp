...
GlobalSlice mlir::pto::insert_sync_frontier::recoverFrontierGlobalSlice(
...
OccurrenceProof mlir::pto::insert_sync_frontier::compareFrontierOccurrences(
...
bool mlir::pto::insert_sync_frontier::localFrontierOverlap(const BaseMemInfo& a, const BaseMemInfo& b)
{
    if (
        a.scope != b.scope) {
        return false;
    }
    if (!a.hasKnownPhysicalAddresses || !b.hasKnownPhysicalAddresses || a.aliasesUnknownRange ||
        b.aliasesUnknownRange || !a.allocateSize || !b.allocateSize || a.baseAddresses.empty() ||
        b.baseAddresses.empty()) {
        return true;
    }
    for (uint64_t x : a.baseAddresses) {
        for (uint64_t y : b.baseAddresses) {
            if (
                insert_sync_detail::addressRangesMayOverlap(x, a.allocateSize, y, b.allocateSize)) {
                return true;
            }
        }
    }

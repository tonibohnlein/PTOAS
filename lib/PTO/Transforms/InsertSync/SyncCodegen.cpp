...
static void MergeSyncList(SyncOps &dstList, const SyncOps &srcList, bool preserve) {
  for (auto *sync : srcList) {
    if (sync->uselessSync) {
      continue;
    }
    if (preserve || !IsSyncExist(dstList, sync)) {
...
void SyncCodegen::UpdateCompoundOpInsertSync(CompoundInstanceElement *nowCompound) {
  ...
  MergeSyncList(pipeBuild.pipeBefore, nowCompound->pipeBefore, preserveCommandWords_);
  MergeSyncList(pipeBuild.pipeAfter, nowCompound->pipeAfter, preserveCommandWords_);
}
...
void SyncCodegen::UpdateLoopOpInsertSync(LoopInstanceElement *nowElement) {
  if (nowElement->getLoopKind() == KindOfLoop::LOOP_END) {
    ...
    MergeSyncList(pipeBuild.pipeBefore, loopBegin->pipeBefore, preserveCommandWords_);
    MergeSyncList(pipeBuild.pipeAfter, nowElement->pipeAfter, preserveCommandWords_);
  }
}
...
void SyncCodegen::UpdateBranchOpInsertSync(BranchInstanceElement *nowElement) {
  if (nowElement->getBranchKind() == KindOfBranch::IF_END) {
    ...
    auto &pipeBuild = op2InsertSync[nowElement->elementOp];
    MergeSyncList(pipeBuild.pipeBefore, branchBegin->pipeBefore, preserveCommandWords_);
    MergeSyncList(pipeBuild.pipeAfter, nowElement->pipeAfter, preserveCommandWords_);
  }
}
...
void SyncCodegen::updatePlaceHolderOpInsertSync(PlaceHolderInstanceElement *placeHolder) {
  ...
  MergeSyncList(pipeBuild.pipeBefore, placeHolder->pipeBefore, preserveCommandWords_);
  MergeSyncList(pipeBuild.pipeAfter, placeHolder->pipeAfter, preserveCommandWords_);
}
...
void SyncCodegen::CreateBarrierOp(IRRewriter &rewriter, Operation *op,
  ...
  if (!preserveCommandWords_ && hasNeighborBarrier(block, ip, currentPipeAttr, insertAtPos)) {
    return;
  }

...
const StorageLifecycle& RequirementFrontiers::lifetime(Cut site, unsigned cell) const
{
    static const StorageLifecycle unknown;
    if (!ready || !use(site, cell).roles) {
        return unknown;
    }
    const auto key = std::make_pair(site, cell);
    ...
    return lifetimes.emplace(key, storage->lifecycleAt(site, cell)).first->second;
}
...
Cut RequirementFrontiers::recurringRelease(Cut site, unsigned cell) const
{
    ...
    while (!todo.empty()) {
        ...
        if (control->sourceCut(at, source.pipe)) {

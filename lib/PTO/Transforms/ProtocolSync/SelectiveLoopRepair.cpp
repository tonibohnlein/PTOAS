...
bool alreadySupplied(
    const StructuredSyncIR& schedule, const SyncSelectedWorld& world, unsigned source, unsigned target,
    const SyncIterationRelation& relation)
{
    auto supply = buildSelectiveLoopSupply(schedule, world);
    return succeeded(supply) && supply->covers(source, target, relation);
}
...
void emitSelectiveAction(OpBuilder& builder, scf::ForOp loop, const SyncSelectiveLoopEdge& edge, bool signal)
{
    const auto kind = edge.completion.iteration.kind;
    const auto location = signal ? edge.placement.source->getLoc() : edge.placement.target->getLoc();
    Value condition;
    if (kind == SyncIterationRelationKind::LoopEntry || kind == SyncIterationRelationKind::LoopExit) {
        const bool outside = kind == SyncIterationRelationKind::LoopEntry ? signal : !signal;
        if (outside) {
            condition = builder.create<arith::CmpIOp>(
                location, arith::CmpIPredicate::slt, loop.getLowerBound(), loop.getUpperBound());
        } else if (kind == SyncIterationRelationKind::LoopEntry) {
            condition = builder.create<arith::CmpIOp>(
                location, arith::CmpIPredicate::eq, loop.getInductionVar(), loop.getLowerBound());
        } else {
            Value remaining = builder.create<arith::SubIOp>(location, loop.getUpperBound(), loop.getInductionVar());
            condition = builder.create<arith::CmpIOp>(location, arith::CmpIPredicate::ule, remaining, loop.getStep());
        }
    }
    scf::IfOp guarded;
    if (condition) {
        guarded = builder.create<scf::IfOp>(location, condition, false);
    }
    OpBuilder::InsertionGuard insertionGuard(builder);
    if (guarded) {
        builder.setInsertionPoint(guarded.thenBlock()->getTerminator());
    }
    if (signal) {
        emitEvent<SetFlagOp>(builder, edge.placement, location);
    } else if (edge.isEvent()) {
        emitEvent<WaitFlagOp>(builder, edge.placement, location);
    } else {
        builder.create<BarrierOp>(location, PipeAttr::get(builder.getContext(), edge.placement.targetPipe));
    }
}
...
SyncRecurringEventProof proveTokens(const SyncSelectiveLoopPlan& plan)
{
    ...
    for (auto [id, edge] : llvm::enumerate(plan.edges)) {
        ...
        const bool boundary = !edge.isEvent() || !recurring;
        if (boundary) {
            continue;
        }
...
bool fitsConcreteBudget(const StructuredSyncIR& schedule, const SyncSelectiveLoopPlan& plan)
{
    ...
    return plan.edges.size() - barriers <= kSelectiveLoopMaximumChannels && barriers <= kSelectiveLoopMaximumBarriers &&
           ...
           operations + 9 * plan.edges.size() + 1 <= kSelectiveLoopMaximumOperations;
}
} // namespace
...
FailureOr<SyncSelectiveLoopAttempt> mlir::pto::protocol_sync::buildSelectiveLoopRepair(const StructuredSyncIR& schedule)
{
    ...
    if (!carrier) {
        return SyncSelectiveLoopAttempt{
            SyncSelectiveLoopStatus::Unsupported, "requires one ordinary unconditional loop"};
    }
    ...
    std::set<std::tuple<SyncPhaseId, SyncPhaseId, Kind>> pairs;
    for (const auto& requirement : local->requirements) {
        ...
        const bool boundary =
            requirement.iteration.carrier == *carrier &&
            (requirement.iteration.kind == Kind::LoopEntry || requirement.iteration.kind == Kind::LoopExit ||
             requirement.iteration.kind == Kind::LoopBypass);
        if (!within && !carried && !boundary) {
        ...
        pairs.emplace(
            ...
            requirement.iteration.kind == Kind::LoopBypass ? Kind::SameIteration : requirement.iteration.kind);
    }
    ...
    for (const auto& first : schedule.getAccesses()) {
        for (const auto& second : schedule.getAccesses()) {
            const bool firstBody = !schedule.findPhase(first.phase)->iterationDomain.loops.empty();
            const bool secondBody = !schedule.findPhase(second.phase)->iterationDomain.loops.empty();
            if (first.phase >= second.phase && !(firstBody && secondBody)) {
                continue;
            }
            ...
            if (first.phase < second.phase) {
                pairs.emplace(
                    ...
                    firstBody == secondBody ? Kind::SameIteration :
                    secondBody              ? Kind::LoopEntry :
                                              Kind::LoopExit);
            }
            if (firstBody && secondBody) {
                pairs.emplace(first.phase, second.phase, Kind::LoopCarried);
            }
        }
    }
    ...
    SmallVector<std::tuple<SyncPhaseId, SyncPhaseId, Kind>, 16> ordered(pairs.begin(), pairs.end());
    ...
    for (auto [sourceId, targetId, kind] : ordered) {
        const unsigned distance = kind == Kind::LoopCarried ? 1 : 0;
        const SyncIterationRelation relation{kind, distance, kind == Kind::SameIteration ? kInvalidSyncId : *carrier};
        ...
        if (source.pipe == destination.pipe) {
            if (alreadySupplied(schedule, plan.world, sourceId, targetId, relation)) {
                continue;
            }
            ...
            if (kind == Kind::LoopEntry) {
                auto loop = cast<scf::ForOp>(plan.loop);
                const bool available = dominance.dominates(loop.getLowerBound(), source.operation) &&
                                       dominance.dominates(loop.getUpperBound(), source.operation);
                if (!available) {
                    return SyncSelectiveLoopAttempt{
                        SyncSelectiveLoopStatus::Unsupported,
                        "loop bounds must be available at the early entry publication"};
                }
            }
        }
    ...
    func::FuncOp function, const SyncSelectiveLoopPlan& plan)
{
    ...
    // Initial credits must not publish unrelated physical prefix work.
    ...
    SmallVector<Operation*, 16> anchors;
    function.walk([&](Operation* operation) {
        if (llvm::any_of(plan.edges, [&](const auto& edge) {
                return edge.placement.source == operation || edge.placement.target == operation;
            })) {
            anchors.push_back(operation);
        }
    });
    for (Operation* operation : anchors) {
        builder.setInsertionPoint(operation);
        for (const auto& edge : plan.edges) {
            if (edge.placement.target != operation) {
                continue;
            }
            emitSelectiveAction(builder, loop, edge, false);
        }
        builder.setInsertionPointAfter(operation);
        for (const auto& edge : plan.edges) {
            if (edge.placement.source == operation && edge.placement.eventId) {
                emitSelectiveAction(builder, loop, edge, true);
            }
        }
    }
    // Token cleanup must not hold back unrelated suffix computation.

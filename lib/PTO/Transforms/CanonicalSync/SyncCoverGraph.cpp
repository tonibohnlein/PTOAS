...
SyncCoverGraph::addScope(SyncCoverScopeId parent, bool mustExecuteWithinParent,
  ...
  scopes_.push_back(
      {id, parent, mustExecuteWithinParent, timeline, isLoop, std::move(guard)});
...
SyncCoverGraph::addCompletionDominance(SyncCoverNodeId source,
  ...
  const bool invalid = source == completionNode ||
                       sourceDescription.resource != completion.resource ||
                       sourceDescription.order >= completion.order ||
                       !syncCoverGuardsCompatible(sourceDescription.guard,
                                                  completion.guard);
  if (invalid) {
  ...
  const bool missing = position == completion.completionDominatedSources.end() ||
                       *position != source;
  if (missing) {

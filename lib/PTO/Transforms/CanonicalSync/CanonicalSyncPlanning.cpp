...
bool hasCommonExecutionContext(const CanonicalSyncPlan &plan,
                               std::size_t source, std::size_t target) {
  return plan.getNodes()[source].operation->getBlock() ==
         plan.getNodes()[target].operation->getBlock();
}
...
void CanonicalSyncPlanBuilder::reduceForwardDependencies() {
  ...
  for (const auto &entry : groups) {
    ...
    if (entry.first.distance == 0 &&
        hasCommonExecutionContext(plan_, source, target)) {
      requirements.push_back({source, target});
      requirementKeys.push_back(entry.first);
    }
  }
  std::vector<SyncGraphEdge> localFixedEdges;
  llvm::copy_if(plan_.fixedEdges_, std::back_inserter(localFixedEdges),
                [&](const SyncGraphEdge &edge) {
                  return hasCommonExecutionContext(plan_, edge.source,
                                                   edge.target);
                });
  std::vector<bool> keep = reduceCompletionRequirements(
      plan_.nodes_.size(), localFixedEdges, requirements);
  ...
  for (const auto &entry : groups) {
    ...
    if (entry.first.distance != 0 && loop) {
  ...
  for (Operation *loop : loops) {
    ...
    for (const SyncGraphEdge &edge : plan_.fixedEdges_) {
      if (loop->isAncestor(plan_.nodes_[edge.source].operation) &&
          ...
          hasCommonExecutionContext(plan_, edge.source, edge.target)) {
        expandedFixed.push_back(edge);
        expandedFixed.push_back(
            {nodeCount + edge.source, nodeCount + edge.target, edge.kind});
      }
    }
    for (const auto &entry : groups) {
      const std::size_t source = entry.first.source;
      const std::size_t target = entry.first.target;
      const unsigned distance = entry.first.distance;
      Operation *owner = entry.first.owner;
      const bool retained = plan_.dependencies_[entry.second.front()].retained;
      if (!retained) {
        continue;
      }
      if (distance == 0 && loop->isAncestor(plan_.nodes_[source].operation) &&
          loop->isAncestor(plan_.nodes_[target].operation) &&
          hasCommonExecutionContext(plan_, source, target)) {
        expandedFixed.push_back(
            {source, target, SyncGraphEdgeKind::HardwareCompletion});
        expandedFixed.push_back({nodeCount + source, nodeCount + target,
                                 SyncGraphEdgeKind::HardwareCompletion});
      } else if (distance != 0 && owner != loop && owner &&
    ...
    for (const CanonicalSyncNode &source : plan_.nodes_) {
      ...
      for (const CanonicalSyncNode &target : plan_.nodes_) {
        if (source.pipe == target.pipe && loop->isAncestor(target.operation) &&
            source.operation->getBlock() == target.operation->getBlock()) {
          expandedFixed.push_back({source.id, nodeCount + target.id,
    ...
    for (const auto &entry : groups) {
      const std::size_t source = entry.first.source;
      const std::size_t target = entry.first.target;
      if (entry.first.distance != 1 || entry.first.owner != loop ||
          !hasCommonExecutionContext(plan_, source, target)) {
        continue;
      }
      recurrenceRequirements.push_back({source, nodeCount + target});
      recurrenceKeys.push_back(entry.first);
    }
    std::vector<bool> recurrenceKeep = reduceCompletionRequirements(
        nodeCount * 2, expandedFixed, recurrenceRequirements);
...
void CanonicalSyncPlanBuilder::materializeSyncRequirements() {
  ...
  for (const auto &entry : groups) {
    ...
    if (source.pipe == target.pipe) {
      if (hasHardwareCompletion(source.pipe)) {
        continue;
      }
      ...
      const bool duplicate =
          llvm::any_of(plan_.barriers_, [&](const auto &old) {

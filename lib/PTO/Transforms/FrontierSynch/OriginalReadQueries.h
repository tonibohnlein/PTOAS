...
public:
    explicit OriginalReadQueries(const OriginalStructure& structure, const OriginalValueQueries* shared = nullptr)
        : original(&structure),
          ownedValues(shared ? nullptr : std::make_unique<OriginalValueQueries>(structure)),
          values(shared ? shared : ownedValues.get())
    {
    ...
    {
        if (!values->current()) {
            static const OriginalReaderFrontiers invalid;
            return invalid;
        }
        const auto key = std::make_tuple(
            request.owner, request.cell, unsigned(request.reader), unsigned(request.scope), request.begin, request.end);
        ...
        const bool valid = original && request.cell < original->cells.size() &&
                           unsigned(request.reader) < unsigned(PipelineType::PIPE_NUM) && owner != owners.end() &&
                           owner->second;
        if (!valid) {
    ...
    const OriginalReadSegment& segment(
        std::size_t ownerId, std::size_t operation, std::size_t cell, PipelineType reader)
    {
        if (!values->current()) {
            static const OriginalReadSegment invalid;
            return invalid;
        }
    ...
    {
        return qualificationAt(predicateId, operation).available();
    }
        ...
        for (const auto& entry : operationRegions) {
            ...
            for (auto parent = parents.find(entry.second); parent != parents.end();
                ...
                for (const auto& access : op.accesses) {
                    ...
                    const auto key = std::make_tuple(
                        interval.owner, interval.cell, unsigned(interval.reader), interval.begin, interval.end);
...
private:
    ...
    std::size_t intern(ParticipationExpression expression)
    {
        ...
        const auto key = std::make_tuple(
            unsigned(expression.kind), expression.left, expression.right, unsigned(atom.kind), atom.owner,
            atom.parameter, atom.value);
    ...
    {
        if (value.kind == ObservationAtom::OriginalBoolean) {
            value.owner = values->canonicalGuardOwner(value.owner);
        }
    ...
    {
        const auto key =
            std::make_tuple(unsigned(value.kind), value.left, value.right, value.predicate, value.operation);
    ...
    Summary derive(const Region& region, std::size_t cell, PipelineType reader)
    {
        if (region.kind == Region::Operation) {
            ...
            const auto leaf = addFrontier({GuardedReadFrontier::Access, NoControlId, NoControlId, 1, region.operation});
        ...
        if (choice) {
            ...
            return {
                true,
                combine(false, combine(true, yes, children[0].nonempty), combine(true, no, children[1].nonempty)),
        ...
        if (counted) {
            ...
            const bool hasLoopSite =
                region.originalOwner < original->originalSites.size() && original->originalSites[region.originalOwner];
            if (!hasLoopSite) {
            ...
            auto loop = dyn_cast<scf::ForOp>(loopSite);
            if (!loop || !values->counted(loop).executableAfterPrerequisites()) {
                return unknown("counted read interval lacks qualified original scalar arithmetic");
            }
            const auto participation = values->predicate(
                body.nonempty, [&](std::size_t id) { return predicate(id); }, values->before(loopSite));
            if (body.nonempty != 1 && !participation.executableAfterPrerequisites()) {
                return unknown("reader participation is not proved invariant across iterations");
            }
            const auto visits =
                region.zeroTripPossible ? atom({ObservationAtom::LoopNonEmpty, region.originalOwner, 0, 1}) : 1;
    ...
    {
        return values->predicate(id, [&](std::size_t node) { return predicate(node); }, values->before(at)).available();
    }
    bool available(const OriginalReaderFrontiers& answer)
    {
        ...
        for (auto [root, after] : {std::make_pair(answer.first, false), std::make_pair(answer.last, true)}) {
            if (root >= frontiers.size()) {
                return false;
            }
            ...
            while (!pending.empty()) {
                ...
                if (frontier.kind == GuardedReadFrontier::Access) {
                    ...
                    if (!qualificationAt(condition(root, frontier.operation), frontier.operation, after).available()) {
                        return false;
                    }
    ...
    std::map<std::tuple<std::size_t, std::size_t, std::size_t, unsigned>, OriginalReadSegment> segments;
    ...
    using QueryKey = std::tuple<std::size_t, std::size_t, unsigned, unsigned, std::size_t, std::size_t>;

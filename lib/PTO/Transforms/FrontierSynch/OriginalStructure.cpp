...
#include "OriginalWriteCoverage.h"
...
namespace {
LogicalResult importControl(
    func::FuncOp function, const SyncInput& input, SyncSlotMapping::AnalysisContext& scalarFacts,
...
std::optional<PhysicalAddressRelation> deriveAddress(
    const BaseMemInfo& memory, SyncSlotMapping::AnalysisContext& scalarFacts,
    const DenseMap<mlir::Operation*, std::size_t>& owners)
{
    if (memory.scope == AddressSpace::GM || memory.scope == AddressSpace::Zero || !memory.allocateSize ||
        memory.aliasesUnknownRange || memory.baseAddresses.empty() || memory.hasKnownPhysicalAddresses) {
        return {};
    }
    ...
    for (auto loop = alloc->getParentOfType<scf::ForOp>(); loop; loop = loop->getParentOfType<scf::ForOp>()) {
    ...
    OriginalStructure& result)
{
    ...
    auto groupFor = [&](const BaseMemInfo* original) {
        ...
        if (carried || (origin != origins.end() && origin->second.unknown)) {
            ...
            memory->baseAddresses.erase(
                std::unique(memory->baseAddresses.begin(), memory->baseAddresses.end()), memory->baseAddresses.end());
    ...
    OriginalWriteCoverage coverage(*result.descriptors);
    for (auto [i, operation] : llvm::enumerate(result.operations)) {
        for (const auto* memory : operation.instruction->defVec) {
            ...
            // Use the private, origin-qualified geometry, not the unwidened input
            // address. The original translated witness is retained independently.
            const auto proof = coverage.qualify(operation, *memories[group]);
            groups[group].uses.push_back({i, false, true, false, memory, groupRelations[group], proof});
        }

...
// License. Shared original-control SCC query; no selected commands or causal credit.
#ifndef PTO_FRONTIERSYNCH_CONTROL_COMPONENTS_H
#define PTO_FRONTIERSYNCH_CONTROL_COMPONENTS_H
#include "Control.h"
#include <algorithm>

namespace mlir::pto::frontiersynch::detail {
// Iterative DFS avoids making compiler stack depth depend on source nesting.
inline std::vector<std::size_t> finishingOrder(const ControlGraph &graph,
...
strongComponents(const ControlGraph &graph,
  ...
  membership.assign(graph.sites.size(), NoControlId);
  for (auto at = order.rbegin(); at != order.rend(); ++at) {
    if (membership[*at] != NoControlId) {
      continue;
    }
    ...
    while (!todo.empty()) {
      ...
      for (auto before : predecessors[site]) {
        if (reachable[before] && membership[before] == NoControlId) {

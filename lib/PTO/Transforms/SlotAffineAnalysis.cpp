...
enumerateSlotSSAOrdinalPairsImpl(Value a, Value b, uint32_t N,
  ...
  SlotForm first;
  SlotForm second;
  const bool invalidForm = !extractSlotForm(a, N, first) ||
                           !extractSlotForm(b, N, second) || first.N != N ||
                           second.N != N || !first.normalizedModuloN ||
  ...
  if (firstSymbolic && secondSymbolic && first.innerSym != second.innerSym) {
    return std::nullopt;
  }
  if (rhsSymbolOffset != 0) {
    const bool incompatibleShift =
        (firstSymbolic || secondSymbolic) &&
        (!shiftedSymbol || (firstSymbolic && first.innerSym != shiftedSymbol) ||

...
// License.
#ifndef PTO_FRONTIERSYNCH_SYNCORIGINPROPAGATION_H
#define PTO_FRONTIERSYNCH_SYNCORIGINPROPAGATION_H
#include <cstddef>
#include <deque>
#include <vector>
...
template <typename Node> std::size_t propagateSyncOrigins(std::vector<Node> &nodes) {
  ...
  while (!queue.empty()) {
    ...
    delta.swap(pending[i]);
    const bool unknown = pendingUnknown[i];
    pendingUnknown[i] = false;
    for (unsigned user : nodes[i].users) {
      auto &target = nodes.at(user);
      for (unsigned root : delta) {
        ++visits;
        if (target.roots.insert(root).second) {
          pending[user].push_back(root);
        }
      }
      if (unknown && !target.unknown) {
        target.unknown = true;
        pendingUnknown[user] = true;
      }

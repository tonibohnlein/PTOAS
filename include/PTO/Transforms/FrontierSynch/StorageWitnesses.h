...
WitnessCounts appendCanonicalStorage(OriginalStructure& p, const std::vector<FootprintGroup>& groups, MayAlias mayAlias)
{
    ...
    auto add = [&](Cell cell, const std::set<std::size_t>& members) {
        ...
        for (auto g : members) {
            for (const auto& u : groups[g].uses) {
                const auto& atom = p.cells.back();
                const bool covered = exact && atom.ranges.size() == 1 &&
                                     u.coverage.covers(atom.ranges.front().first, atom.ranges.front().second);
                p.operations.at(u.operation)
                    .accesses.push_back(
                        {id, u.read, u.write, exact && u.write && (u.definiteWrite || covered), u.memory,
                         u.physicalRelation, u.coverage});
            }
        }
    };

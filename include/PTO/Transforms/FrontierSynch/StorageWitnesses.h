...
// License.
#ifndef PTO_FRONTIERSYNCH_STORAGEWITNESSES_H
#define PTO_FRONTIERSYNCH_STORAGEWITNESSES_H
...
WitnessCounts appendCanonicalStorage(OriginalStructure &p,
  ...
  auto add = [&](Cell cell, const std::set<std::size_t> &members) {
    const auto id = p.cells.size();
    const bool exact = cell.storage == Cell::Storage::CanonicalInterval;
    for (auto g : members) {
      cell.storageOrigins.insert(cell.storageOrigins.end(),
                                 groups[g].description.storageOrigins.begin(),
                                 groups[g].description.storageOrigins.end());
    }
    std::sort(cell.storageOrigins.begin(), cell.storageOrigins.end());
    cell.storageOrigins.erase(std::unique(cell.storageOrigins.begin(), cell.storageOrigins.end()),
                              cell.storageOrigins.end());
  ...
  for (std::size_t g = 0; g < groups.size(); ++g) {
    ...
    qualified[g] = !c.exclusive && !c.unknownRange && !c.coordinateSpace.empty() &&
                   c.ranges.size() == 1 && c.ranges[0].second &&
  ...
  for (std::size_t a = 0; a < groups.size(); ++a) {
    for (std::size_t b = a + 1; b < groups.size(); ++b) {
      ...
      if (qualified[a] && qualified[b] &&
          x.description.coordinateSpace == y.description.coordinateSpace) {
      ...
      witness.ranges.insert(witness.ranges.end(), y.description.ranges.begin(),
                            y.description.ranges.end());

...
def check_domains(isl, data, oracle, n, flip):
    ...
    assert symbols == (2 if oracle.options.get("skipped_reader") else 1), symbols
    params = ",".join(f"p{s}" for s in range(symbols))
    bindings = f"p0={n}" + (f" and p1={flip}" if symbols == 2 else "")
    ...
    for key in universe:
        _, p, q, _, _ = key
        scope = isl.map(f"[{params}] -> {{[{p},i] -> [{q},j]: {bindings} and 0<=i<{n} and 0<=j<{n}}}")
        pairs = expected.get(key, set())
        clauses = "; ".join(f"[{p},{i}] -> [{q},{j}]: {bindings}" for i, j in sorted(pairs))
...
def inspect(data, source, spec, isl, ir, pto, require_quality):
    ...
    rows, independent, broadened = [], 0, 0
    with ir.Context() as context:
        ...
        return {"static": static_metrics(candidate.operation), "scenarios": rows,
...
def main():
    ...
    def invoke(path, mutation, label):
        command = [str(args.driver.resolve()), str(path.resolve()), mutation]
        start = time.monotonic()
        try:
            result = subprocess.run(command, text=True, capture_output=True, timeout=90, env=env)
            ...
            raise RuntimeError(f"{label}: strict native invocation exceeded 90 seconds") from error
    ...
    for spec in specs:
        ...
        row = {"case": spec["name"], "source_sha256": spec["sha256"], "strict_status": data["status"],
               "reason": data["reason"], "work": data["work"], "applied": data["applied"]}
        rows.append(row)
        if not data["applied"]:
            ...
            assert spec["name"] not in MANDATORY, row
        ...
        if spec["name"] == "rotating_d2" and args.mutations:
            row["mutations"] = []
            for mutation in ("erase-wait", "swap-loads", "swap-wait-keys"):
    ...
    summary = {"status": "passed", "cases": rows, "applied": sum(r["applied"] for r in rows),
               "requested": len(rows), "mandatory_applied": sorted(MANDATORY & {r["case"] for r in rows if r["applied"]}),

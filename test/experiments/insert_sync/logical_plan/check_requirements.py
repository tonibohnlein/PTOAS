...
def main():
    ...
    for name, source in samples.items():
        ...
        assert len(accesses) == len(data["accesses"]), "duplicate access identity"
        phases = data["phases"]
        assert [p["id"] for p in phases] == list(range(len(phases)))
        assert len(data["orders"]) == len(data["points"]) ** 2
        assert {(e["source"], e["target"]) for e in data["orders"]} == {
            (p, q) for p in range(len(data["points"])) for q in range(len(data["points"]))}

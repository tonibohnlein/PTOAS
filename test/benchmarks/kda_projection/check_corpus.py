...
def main():
    ...
    parser.add_argument("--accepted-cases", type=Path,
                        help="Prior manifest results; select ids whose rc is zero")
    parser.add_argument("--migrate-scalar-accesses", "--migrate-scalar-loads", dest="migrate_scalar_loads", action="store_true",
                        help="Migrate obsolete scalar load/store spelling identically for both arms")
    parser.add_argument("--out", type=Path, required=True)
    parser.add_argument("--jobs", type=int, default=2)
    parser.add_argument("--timeout", type=int, default=180)
    args = parser.parse_args()
    assert 1 <= args.jobs <= 2 and args.timeout > 0
    args.out.mkdir(parents=True, exist_ok=False)
    manifest = json.loads((args.package / "manifest.json").read_text())
    cases = [c for c in manifest["cases"] if c["target"] == "a3"]
    if args.accepted_cases:
        names = {c["id"] for c in json.loads(args.accepted_cases.read_text()) if c["rc"] == 0}
    ...
    def run(case):
        ...
        inp = (args.package / case["input"]).resolve()
        if args.migrate_scalar_loads:
            text, count = re.subn(r"pto\.load_scalar (.*?) : <(.*?)> ->",
                                  r"pto.load \1 : !pto.ptr<\2> ->", inp.read_text())
            text, stores = re.subn(r"pto\.store_scalar (.*?) : <(.*?)>,",
                                   r"pto.store \1 : !pto.ptr<\2>,", text)
            row["scalar_store_spelling_migrations"] = stores
            inp = args.out.resolve() / case["id"] / "input.pto"
            inp.parent.mkdir(parents=True, exist_ok=True)
            inp.write_text(text)
            row["scalar_spelling_migrations"] = count
        row["evaluated_input_sha256"] = sha(inp)
        for arm, driver in drivers.items():
            ...
            flags = args.baseline_flag if arm == "baseline" else args.candidate_flag
            command = [str(driver), "--construct", str(inp), *flags]

...
SyncCoverTargetCapabilities makeA2A3Capabilities() {
...
bool testBudgetOneScarcityFailsClosed() {
  bool passed = true;
  ...
  SyncCoverColumnGenerationContext context{makeA2A3Capabilities(),
                                           scenario.demands};
  ...
  passed &= check(static_cast<bool>(generated),
                  "canonical event generation succeeds");
...
bool testPiercingAndDeterminism() {
  ...
  const auto runShape = [] {
    ...
    localUniverse.addResourceDomain(SyncCoverResourceKind::EventId,
                                    kPipeMTE2, kPipeM, 8);
    const SyncCoverTargetCapabilities localTarget = makeA2A3Capabilities();

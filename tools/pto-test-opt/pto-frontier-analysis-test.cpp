...
#include "FrontierWriteCoverageChecks.h"
#include "FrontierObligationChecks.h"
#include "FrontierObligationProgramChecks.h"
...
mlir::LogicalResult inspectRequests(const fs::ProgramAnalysis& analysis, Counts& counts)
{
    for (std::size_t site = 0; site < analysis.structure().operations.size(); ++site) {
        ...
        for (std::size_t index = 0; index < requests.size(); ++index) {
            ...
            // Consume the actual guarded view, not merely the presence of a DAG.
            // Missing repeated summaries remain visible; they never erase the
            // conservative marginal request being inspected here.
            if (answer.factoredUse != &analysis.originalUsesAt(site, request.relationship.cell) ||
                !answer.factoredDemand.valid || answer.factoredDemand.targetEffects().empty()) {
                return mlir::failure();
            }
            ++counts.interpreted;
            ++counts.hazards.at(static_cast<std::size_t>(request.relationship.kind));
            counts.unknownOccurrence +=
                answer.occurrence.status == fs::OriginalOccurrenceInterpretation::Status::Unknown;
...
mlir::LogicalResult analyze(mlir::func::FuncOp function)
{
    ...
    if (!checkProgramObligations(analysis)) {
        return function.emitError("original obligation universe or witness check failed");
    }
    if (mlir::failed(checkWriteCoverageExpectations(analysis))) {
        return mlir::failure();
    }
    Counts counts;
    if (mlir::failed(inspectRequests(analysis, counts))) {
        return function.emitError("missing original source subscription or factored demand view");
    }
...
int main(int argc, char** argv)
{
    ...
    if (selfTest) {
        if (!checkFactoredNativeProvenance() || !frontier_obligation_checks::run()) {
